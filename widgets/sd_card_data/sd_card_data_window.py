...
from widgets.sd_card_data.collapsible_widget import CollapsibleSection
...
class SDCardDataWindow(QWidget):
    def __init__(self, main_window):
        ...
        # Left panel for collapsible sections
        left_panel = QWidget()
        ...
        # Create first collapsible section - SD Card Files
        self.files_section = CollapsibleSection("SD Card Files")
        ...
        # Populate the files tree with sample data
        self.populate_files_tree()
        
        self.files_section.add_widget(self.files_tree)
        left_layout.addWidget(self.files_section)

        # Create second collapsible section - Parameter Monitor
        self.params_section = CollapsibleSection("Parameter Monitor")
        ...
        self.params_section.add_widget(self.tree)
    ...
    def show_files_context_menu(self, position):
        ...
        if item:
            if item.parent() is None:  # Folder item
        ...
        # Handle menu actions (placeholder implementations)
        if item and action:
            if action.text() == "Open":
                print(f"Opening file: {item.text(0)}")
            elif action.text() == "Copy":
                print(f"Copying file: {item.text(0)}")
            elif action.text() == "Delete":
                print(f"Deleting file: {item.text(0)}")
            elif action.text() == "Refresh" or action.text() == "Refresh Folder":
                self.populate_files_tree()
            elif action.text() == "Properties":
                print(f"Properties for: {item.text(0)}")
    ...
    def on_mouse_move(self, event):
        ...
        if closest_info and min_distance < 0.1:  # Within reasonable distance
            title = f"SD Card Data Parameter Monitoring | {closest_info['param']}: {closest_info['value']:.2f} at {closest_info['time'].strftime('%H:%M:%S')}"
